...
if (!class_exists('rcwdNggauthors')){
	class rcwdNggauthors{
			...
			if (in_array($this->depends, $this->active_plugins)){
				...
				if(is_admin()){
					if(isset($_GET['action']) and $_GET['action'] == 'nggauthors-remove-notice'){
						$this->nggauthors_options['nggauthors-remove-notice'] = true;
		...
		function vars_and_constants(){
			global $wpdb;
			define('RCWDNGGAUTHORS_VERSION', '0.2.5');
		...
		function activate(){
			...
			update_option( 'rcwdnggauthors_version', RCWDNGGAUTHORS_VERSION );
		}
		...
		function version_check(){
			$old_rcwdnggauthors_version = get_option('rcwdnggauthors_version');
			if( empty($old_rcwdnggauthors_version) or (version_compare(RCWDNGGAUTHORS_VERSION, $old_rcwdnggauthors_version, '>') )){
				$this->update();
			}
		}
